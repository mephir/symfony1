...
EOF;
  ...
  protected function execute($arguments = array(), $options = array())
  {
    ...
    {
      $file = sprintf('%s%s/%s/Base%s%s', $config['models_path'], isset($definition['package']) ? '/'.substr($definition['package'], 0, strpos($definition['package'], '.') === false ? strlen($definition['package']) : strpos($definition['package'], '.')) : '', $builderOptions['baseClassesDirectory'], $model, $builderOptions['suffix']);
